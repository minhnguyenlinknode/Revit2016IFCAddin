...
namespace Revit.IFC.Export.MinhTest
{
    ...
    public class Command : IExternalCommand
    {
        ...
        private string GenerateFileNameFromDocument(Document doc, ISet<string> exportedFileNames)
        {
            ...
            if (String.IsNullOrEmpty(title))
                title = "DefaultFileName" + "-" + Guid.NewGuid().ToString();
        ...
        {
            Document document2 = commandData.Application.ActiveUIDocument.Document;
            Guid gUID = commandData.Application.ActiveAddInId.GetGUID();
            //UICommand uICommand = new UICommand(document, gUID.ToString());
            //uICommand.ShowDialog();            

            //Revit.IFC.Export.Exporter.Exporter exporter = new Revit.IFC.Export.Exporter.Exporter();            
            //var exporterIFC = (Autodesk.Revit.DB.IFC.ExporterIFC)System.Reflection.Assembly.GetExecutingAssembly().CreateInstance("Autodesk.Revit.DB.IFC.ExporterIFC");
            //exporter.ExportIFC(document, exporterIFC, null);
            ...
            //if (service != null)
            //{
            //    var exporter = new Revit.IFC.Export.Exporter.Exporter();
            //    service.AddServer(exporter);
            //    service.SetActiveServer(exporter.GetServerId());
            //}

            //System.Windows.Forms.MessageBox.Show("Ok. Click Export to IFC to see what going on?");
            ...
            try
            {
                ...
                if (mainWindow.Result == IFCExportResult.ExportAndSaveSettings)
                {
                    ...
                    fileDialog.FileName = multipleFiles ? "MultipleFiles" : GenerateFileNameFromDocument(mainWindow.DocumentsToExport[0], null);
                    ...
                    if (fileDialogResult.HasValue && fileDialogResult.Value)
                    {
                        ...
                        {
                            using (TaskDialog taskDialog = new TaskDialog("IFCExport"))
                            {
                                taskDialog.MainInstruction = string.Format("IFCExportProcessError", unsuccesfulExports);
            ...
            catch (Exception e)
            {
                using (TaskDialog taskDialog = new TaskDialog("IFCExport"))
                {
                    taskDialog.MainInstruction = "IFCExportProcessGenericError";
                    taskDialog.MainIcon = TaskDialogIcon.TaskDialogIconWarning;
        ...
        public void ExportLinkedDocuments(Autodesk.Revit.DB.Document document, string fileName, Dictionary<ElementId, string> linksGUIDsCache, IFCExportOptions exportOptions)
        {
            ...
            foreach (String linkPathName in rvtLinkNamesToInstancesDict.Keys)
            {
                ...
                if (numBadInstances > 0)
                {
                    using (TaskDialog taskDialog = new TaskDialog("IFCExport"))
                    {
                        taskDialog.MainInstruction = string.Format("LinkInstanceExportErrorMain", numBadInstances);
                        taskDialog.MainIcon = TaskDialogIcon.TaskDialogIconWarning;
                        taskDialog.TitleAutoPrefix = false;

                        string expandedContent = "";
                        AddExpandedStringContent(ref expandedContent, "LinkInstanceExportErrorPath", pathDoesntExist);
                        AddExpandedStringContent(ref expandedContent, "LinkInstanceExportCantCreateDoc", noTempDoc);
                        AddExpandedElementIdContent(ref expandedContent, "LinkInstanceExportCantFindDoc", cantFindDoc);
                        AddExpandedElementIdContent(ref expandedContent, "LinkInstanceExportNonConformal", nonConformalInst);
                        AddExpandedElementIdContent(ref expandedContent, "LinkInstanceExportScaled", scaledInst);
                        AddExpandedElementIdContent(ref expandedContent, "LinkInstanceExportHasReflection", instHasReflection);
